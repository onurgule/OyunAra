...
class _FiltersScreenState extends State<FiltersScreen> {
  ...
  List<Widget> getOnlineListUI() {
    final List<Widget> noList = <Widget>[];
    for (int i = 0; i < onlineList.length; i++) {
      final PopularFilterListData date = onlineList[i];
      noList.add(
        Material(
          color: Colors.transparent,
          child: InkWell(
  ...
  void checkAppPositionOn(int index) {
    if (index == 0) {
      ...
      for (int i = 0; i < onlineList.length; i++) {
        if (i != 0) {
          final PopularFilterListData data = onlineList[i];
          if (data.isSelected) {
            count += 1;
          }
        }
      ...
      if (count == onlineList.length - 1) {
        onlineList[0].isSelected = true;
      } else {
        onlineList[0].isSelected = false;
      }
    }
  }
